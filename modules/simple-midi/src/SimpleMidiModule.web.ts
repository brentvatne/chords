...
import { ConnectionInfo, MidiDevice, SimpleMidiModuleEvents } from './SimpleMidi.types';
...
class SimpleMidiModuleWeb extends EventEmitter<SimpleMidiModuleEvents> {
  ...
  async sendNoteOff(note: number, velocity: number, channel: number): Promise<void> {
    console.warn('MIDI is not supported on web platform');
  }

  async sendControlChange(controller: number, value: number, channel: number): Promise<void> {
    console.warn('MIDI is not supported on web platform');
  }
