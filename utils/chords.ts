...
export type MusicalNoteWithOctave = `${MusicalNote}${1 | 2 | 3 | 4 | 5 | 6 | 7 | 8}`;
...
export function mapSelectionToTonalType(selection: ChordSelection): string {
  ...
  switch (selection.triad) {
    case 'dim': base = 'dim'; break;
    case 'minor': base = 'm'; break;
    case 'major': base = 'M'; break;
    case 'sus': base = 'sus4'; break;
  }
...
export function getChordInfo(selection: ChordSelection, root: MusicalNoteWithOctave): ChordInfo | null {
  try {
    ...
    if (!chordData || chordData.empty || !chordData.tonic) {
      ...
      if (selection.extensions.includes('6') && 
        ...
        const baseChord = selection.extensions.includes('M7') 
          ...
          : Chord.getChord(selection.triad === 'minor' ? 'm7' : 
                          (selection.triad === 'dim' ? 'dim7' : 
                          (selection.triad === 'sus' ? '7sus4' : '7')), root);
      ...
      if (selection.triad === 'sus' && selection.extensions.includes('6')) {
        ...
        if (!baseChord.empty && baseChord.tonic) {
          ...
          if (selection.extensions.includes('m7')) {
            ...
            const seventhIndex = notes.findIndex(note => note === seventhNote);
            if (seventhIndex !== -1) {
      ...
      if (selection.extensions.includes('M7') && selection.extensions.includes('9')) {
        ...
        if (!baseChord.empty && baseChord.tonic) {
          ...
          if (selection.extensions.includes('6')) {
            ...
            const seventhIndex = notes.findIndex(note => note === Note.pitchClass(Note.transpose(root, 'M7')));
            if (seventhIndex !== -1) {
              notes.splice(seventhIndex, 0, Note.pitchClass(sixth));
            } else {
              notes.push(Note.pitchClass(sixth));
      ...
      console.warn(`No chord found for ${root} (selection: ${JSON.stringify(selection)}, tried types: ${tonalType}${alternativeType ? ', ' + alternativeType : ''})`);
      return null;
    }
...
function orderChordNotes(notes: string[], root: string, selection: ChordSelection): string[] {
...
export function chordToMidiNotes(selection: ChordSelection, root: MusicalNoteWithOctave): number[] {
  try {
    const tonalType = mapSelectionToTonalType(selection);
    const chordData = Chord.getChord(tonalType, root);
    if (chordData.empty) {
      console.warn(`Cannot get MIDI notes for ${root} (selection: ${JSON.stringify(selection)}, tonalType: ${tonalType}): chord data is empty.`);
      return [];
    }
    ...
    if (midiNotes.length === 0 && chordData.notes.length > 0) {
      console.warn(`Could not convert any notes to MIDI for ${root} with selection ${JSON.stringify(selection)}. Original notes: ${chordData.notes.join(',')}`);
    }
    return midiNotes;
  } catch(e) {
